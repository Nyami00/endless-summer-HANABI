...
| 項目 | 値 | 意味 |
|---|---|---|
| f0 | `mix(2600, 1400, s)` | 開始周波数。小玉ほど高い（2026-09-06 聴感調整: 「お化け屋敷のよう」との指摘を受け、高く短い設計に変更） |
| f1 | `f0 * 2^(-mix(4, 7, s) / 12)` | 終了周波数。上昇とともに 4〜7 半音下がる（遠ざかる感じ） |
| dur | `clamp(duration * 0.4, 0.6, 2.2)` | 持続時間。上昇時間の 4 割で、開花のかなり前に消える |
| attack / release | `0.05` / `0.05` | 立ち上がり・止め際 |
| decayTo | `0.1` | dur 時点の音量比（指数減衰で「ヒュッ…」と消える） |
| noiseMix | `mix(0.5, 0.7, s)` | 息（帯域ノイズ）成分の比率。残りが笛（正弦波）成分。ビブラートは付けない |
| gain | `mix(0.07, 0.12, s) * dist` | 全体音量。ドンより控えめ |
| q | `12` | 帯域ノイズのバンドパス Q |
| breathGain | `2.5` | 息成分の補正係数（バンドパスで落ちる分を補う）。builder 側に裸の定数を置かない |
| reverbSend | `0.4` | |
...
- ヒュー: 正弦波オシレーター（周波数を f0 → f1 へ下降する指数ランプ。ビブラートなし）と、ノイズバッファ → バンドパス（中心周波数を同じ軌道でランプ、Q 14）の2系統を `noiseMix` で混ぜ、包絡ゲイン → パン → master と reverb へ。パンは発射台の位置で固定（上昇中の追従はしない）
...
- 到達時刻は既存どおり `simTime + d / 343`。ヒューは発射台から、ドンは開花位置から届くため、ヒューは上昇の途中（開花のかなり前）で消え、ドンは開花位置から遅れて届くので、開花の前後に自然な静寂ができる
...
- ヒュー: 開始側 0.3〜0.8s のゼロ交差率 > 終了側 (dur−0.8)〜(dur−0.3)s のゼロ交差率（音程が下がっている）
